  ...
  <section id="space-section">
    ...
    <div id="section-main-content">
      ...
      <div id="slider-container">
        <ul
          id="slider"
          :class="{ scroll: slideIndex != 0 }"
          :style="{ '--current-slide': slideIndex }"
        >
...
ul#slider {
  ...
  --width-void-space: calc(
    100vw - (var(--button-width-space) * 2) - var(--slide-width)
  );
  --slides-gap: 55px;
  --slides-offset: 7%;
  ...
  display: flex;
  padding: 0px
    calc(var(--button-width-space) + calc(var(--width-void-space) / 2));
  gap: var(--slides-gap);
  align-items: center;
...
ul#slider.scroll {
  transform: translateX(
    calc(
      calc((var(--slide-width) + var(--slides-gap))) * var(--current-slide) * -1
    )
  );
}
