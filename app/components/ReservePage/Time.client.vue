...
onMounted(() => {
  nextTick(() => {
    // This ensures the DOM has been updated before running the logic
        ...
        (entries) => {
          ...
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              currentMinute.value = value.startsWith("0")
                ? value.slice(1).trim()
                : value;
            }
          });
...
<template>
  <article id="time-container">
    ...
    <div id="time-ctrls-container">
      ...
      <ul id="minute-list" class="time-list" ref="minuteListEl">
        ...
        <li
          ...
          :class="{
            'minute-item': true,
            selected: minute == Number(currentMinute),
          }"
        >
      ...
      <select
        id="day-part"
        @change="dayPart = ($event.target as HTMLSelectElement).value"
      >
    ...
    <button
      id="next-btn"
      @click="$emit('set-time', currentHour, currentMinute, dayPart)"
    >
      Siguiente
    </button>
  </article>
